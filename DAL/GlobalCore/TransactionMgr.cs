...
{
    /// <summary>
    /// TransactionMgr is used to seemlessly enroll CoreEntity's  into a transaction. TransactionMgr uses
    /// ADO.NET transactions and therefore is not a distributed transaction as you would get with COM+. You only have
    /// to use TransactionMgr if two or more CoreEntity's need to be saved as a transaction.  The CoreEntity.Save
        ...
        public void CommitTransaction()
        {
            ...
            if (txCount == 0)
            {
                ...
                if (this.objectsInTransaction != null)
                {
                    try
                    {
                        foreach (CoreEntity entity in this.objectsInTransaction)
                        {
